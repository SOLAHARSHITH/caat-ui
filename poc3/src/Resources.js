...
const Resources = ({ open }) => {
  ...
  const onConnect = 
    (params) =>{
      ...
      const sourceNodeNew = nodes.find((node) => node.id === source);
      const targetNodeNew = nodes.find((node) => node.id === target);
      // const isSourceClosest = sourceNode.position.x < targetNode.position.x;
      // const source_id= isSourceClosest ? source : target;
      // const target_id= isSourceClosest ? target : source;
      // const sourceNodeNew = nodes.find((node) => node.id === source_id);
      // const targetNodeNew = nodes.find((node) => node.id === target_id);
  
      if (sourceNodeNew && targetNodeNew) {
        const sourceJobId = sourceNodeNew.data.job_id;
        const targetJobId = targetNodeNew.data.job_id;
        console.log(typeof(sourceJobId));
      console.log(targetJobId);
      let newEdge;
        if (sourceJobId >= targetJobId) {
          const [SourceEdgeid,TargetEdgeid]=SwapNodes(sourceNodeNew,targetNodeNew);
            newEdge = {
              id: `edge-${SourceEdgeid}-${TargetEdgeid}`,
              source:SourceEdgeid,
              target:TargetEdgeid,
              type: 'default',
              markerEnd: {
  ...
  const SwapNodes=(sourceNode,targetNode)=>{
    ...
    const updatedSourceNode = {
        ...sourceNode,
        data: { ...sourceNode.data, job_id: `${targetJobId}` ,label_ID: `${targetJobId}`},
        id: `${targetJobId}`
      };
      ...
      const updatedTargetNode = {
        ...targetNode,
        data: { ...targetNode.data, job_id: `${sourceJobId}` ,label_ID: `${sourceJobId}`},
        id: `${sourceJobId}` 
      };
      ...
      const SourceEdgeid=SourceCorrection(updatedSourceNode.id);
      const TargetEdgeid=TargetCorrection(updatedTargetNode.id);
      return [SourceEdgeid,TargetEdgeid];
  ...
  if (Required_Node) {
    console.log("target correction")
      ...
      if (targetNodes.length > 0) {
          ...
          console.log(typeof(mintargetNodeId));
...
const getTargetNodes = (nodeId) => {
  ...
  const onDrop = 
    (e) => {
      ...
      const newItem = {
        id: `${job_id}`,
        type: type,
        data: { label: ` ${labelName}`, label_ID: `${job_id}` ,job_id, nodeData, tempinputValues,variables:{
          "assetID": "",
          "packageID": "",
          "input": [
              {
  ...
  const getNodeByAssetID = (id) => {
    ...
    const tempNodeVariableData = targetNode.data.variables.input;
                  ...
                  <div className="ml-5 mr-5">
                    {list &&
                      Object.values(list).map((keys) => (
                        <div
                          className="ls my-2  border-2 border-black hover:bg-black hover:border-white hover:text-white rounded-lg p-2 cursor-grab"
                          onDragStart={(event) => {
